...
export default async function NewPostPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <form action={createAction} className="space-y-4 rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-slate-900 p-4 shadow-sm">
        <TextInput name="title" label="标题" required placeholder="文章标题" />
        <TextInput name="slug" label="Slug" required placeholder="url-friendly-slug" />
        <TextInput name="date" label="日期" type="date" defaultValue={new Date().toISOString().slice(0, 10)} />
        <TextInput name="tags" label="标签（逗号分隔）" placeholder="tag1, tag2" />
        <CoverField label="封面 URL" />
        <TextInput name="summary" label="摘要" placeholder="一句话摘要" />
        <ContentWithPreview />
        <div className="space-y-2">
